    ...
    groups+: [
      {
        ...
        rules: [
          {
            expr: 'label_replace( (sum by (namespace, container, pod, environment) ( sum(container_memory_usage_bytes{container!=""}) by (container, pod, namespace) * on (pod, namespace) group_left(environment) label_replace(kube_pod_labels{label_app!=""},"pod","$1","pod","(.*)") ) / sum( label_replace( label_replace( kube_pod_container_resource_limits_memory_bytes{container!=""}, "container", "$1", "container",  "(.*)" ), "pod", "$1", "pod", "(.*)" ) ) by (namespace, container, pod, environment) * 100), "service", "$1", "container", "(.*)")',
